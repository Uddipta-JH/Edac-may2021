...
select a.employee_id,a.last_name,b.manager_id,b.LAST_NAME from employees b,employees a
where a.manager_id=b.employee_id; 

-- 5.Write a query to find the name (first_name, last_name) and hire date of the employees who was hired after 'Jones.
select FIRST_NAME,LAST_NAME from employees where HIRE_DATE >= any  (select HIRE_DATE from employees where LAST_NAME='Jones');
...
select employees.EMPLOYEE_ID,jobs.JOB_TITLE ,datediff(job_history.END_DATE,job_history.Start_date) from  job_history,jobs,employees
where employees.department_id=90 and employees.JOB_ID=jobs.JOB_ID and job_history.EMPLOYEE_ID=employees.EMPLOYEE_ID group by EMPLOYEE_ID;

-- 8.Write a query to display the department ID and name and first name of manager. 
select departments.DEPARTMENT_ID,DEPARTMENT_NAME,first_name from departments,employees where departments.MANAGER_ID=employees.EMPLOYEE_ID;

-- 9.Write a query to display the department name, manager name, and city.
select department_name,concat(first_name,' ',last_name), city from departments,employees,locations where 
departments.MANAGER_ID=employees.EMPLOYEE_ID and locations.LOCATION_ID = departments.LOCATION_ID;
																								
-- 10.Write a query to display the job title and average salary of employees.
select job_title, avg(salary) from jobs,employees where jobs.JOB_ID=employees.JOB_ID group by JOB_TITLE;
...
select job_title,concat(first_name,' ',last_name) Employee_name, salary-min_salary from jobs,employees where jobs.job_id=employees.job_id group by Employee_name;

-- 12 Write a query to display the job history that were done by any employee who is currently drawing more than 10000 of salary.
select *from job_history where job_history.EMPLOYEE_ID=any(select EMPLOYEE_ID from employees where salary>10000);

-- 13.Write a query to display department name, name (first_name, last_name), hire date, salary of the manager for all managers whose experience is more than 15 years
select department_name,concat(a.first_name,' ',a.last_name) name,a.hire_date,a.salary from departments,employees a,employees b where departments.department_id=a.department_id and
a.manager_id=b.employee_id and (sysdate()-a.hire_date)/365>15 group by department_name;
